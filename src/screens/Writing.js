...
class Writing extends Component {
	...
	onSave = async () => {
		...
		try {
			const { data } = await http.putAPI(
				'/v2/stories/' + encodeURIComponent(storyId),
				{
					title: title,
					content: content,
				},
	...
	onBack = () => {
		...
		Alert.alert(
			translate('writing.unsaved.title'),
			translate('writing.unsaved.message'),
			[
				{
					text: 'Cancel',
					onPress: () => {
	...
	render() {
		...
		const topBar = (
			<TopBar title='writing.topbar' onBack={this.onBack} action={action} />
		);
